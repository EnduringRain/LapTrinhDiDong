...
class _CreateReservationScreenState extends State<CreateReservationScreen> {
  ...
  Future<void> _createReservation() async {
    ...
    if (_formKey.currentState!.validate()) {
      ...
      if (isToday && _selectedTable!.trangThai != "Trống") {
        QuickAlertService.showAlertWarning(context, "Bàn ${_selectedTable!.ma} đang ở trạng thái ${_selectedTable!.trangThai}, không thể đặt cho hôm nay.");
        await Future.delayed(const Duration(seconds: 2));
        return;
      }


      final bool isWithinNextHour = reservationDateTime.isAfter(now) &&
  ...
  Widget build(BuildContext context) {
    bool isConfirmButtonAlwaysEnabled = _selectedTable != null && _selectedDate != null && _selectedTime != null;
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Form(
          ...
          child: ListView(
            children: [
              Padding(padding: EdgeInsets.only(bottom: 10), child: Text('Thông tin khách hàng', style: Theme.of(context).textTheme.titleLarge),),
              TextFormField(
              ...
              Padding(padding: EdgeInsets.only(bottom: 10), child: Text('Thông tin đặt bàn', style: Theme.of(context).textTheme.titleLarge),),
              InkWell(
              ...
              StreamBuilder<List<Ban>>(
                ...
                builder: (context, snapshot) {
                  ...
                  if (isToday) {
                    displayTables = allTables.where((ban) => ban.trangThai == "Trống" && ban.ma != null).toList();
                  } else {                 
                    displayTables = allTables.where((ban) => ban.ma != null).map((ban) {
                      return Ban(
                        ma: ban.ma,
                        sucChua: ban.sucChua,
                        viTri: ban.viTri,
